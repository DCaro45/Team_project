...
model_functs = [linear_S, linear_S_decay, linear_kelvin, Burger]

for func in model_functs:
    model_funct = func  # choose the model function to fit the data
    ...
    for k, T in enumerate(Temp[a:b+1]):
        ...
        for j, n in enumerate(sensor_number[c:d+1]):
            ...
            #print('errors are = {} , {}'.format(*popt_errs))
    ...
    c = 0
    d = 10
    cmaps = ['Greys', 'Purples', 'Blues', 'Greens', 'Oranges', 'Reds', 'YlOrBr', 'RdPu', 'BuPu', 'GnBu', 'YlGn']
    for j, n in enumerate(sensor_number[c:d+1]):
        j = j + c
        Ts = []
        fig1 = plt.figure(j, figsize=[12,6])

        ax1 = fig1.add_axes((0.07, 0.295, 0.775, 0.65))
        ...
        ax1.set_title('Sensor no.' + str(n))

        ax2 = fig1.add_axes((0.07, 0.1, 0.775, 0.17))
        ...
        ax3 = fig1.add_axes((0.857, 0.1, 0.135, 0.17))
        ...
        cm = plt.get_cmap(cmaps[j])(np.linspace(0.25, 0.8, len(Temp)))
        print(cm)
        ...
        for k, T in enumerate(Temp[a:b+1]):
            ...
            ax1.plot(xs, ys, color=cm[k], linestyle='dashed')
            ax1.scatter(t, R, s=0.5, label=str(T) + 'C', color=cm[k])
            ax2.plot(t, Res, color=cm[k])
            ax3.hist(Res, bins=50, orientation='horizontal', density=True, rwidth=0.7, color=cm[k], alpha=0.7)
            ax3.plot(norm.pdf(x_hist, 0, std), x_hist, color=cm[k], linestyle='dashed', alpha=0.5)

        ax2.axhline(y=0, linestyle='dashed', color='black', linewidth='1')
        ax2.axhline(y=2.5, color='darkgrey', linewidth='1')
        ax2.axhline(y=-2.5, color='darkgrey', linewidth='1')
        ax2.axhspan(-2.5, 2.5, color='lightgrey', alpha=0.3)

        #ax3.axhline(y=0, linestyle='dashed', color='black', linewidth='1')
        ax3.axhline(y=2.5, color='darkgrey', linewidth='1')
        ax3.axhline(y=-2.5, color='darkgrey', linewidth='1')
        ax3.axhspan(-2.5, 2.5, color='lightgrey', alpha=0.3)
        ...
        plt.savefig(dir + '\\Graphs\\' + str(n) + '.png')
        plt.show()
        #plt.figure(figsize=[6, 6])
        #plt.scatter(Ts, red_chi_sq[:, j])
        #plt.show()

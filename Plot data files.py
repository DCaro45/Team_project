...
for k, T in enumerate(Temp[a:b+1]):
    ...
    for j, n in enumerate(sensor_number[c:d+1]):
        j = j + c
        file = (dir + "\\All Data\\" + str(T) + " Temp\\" + sensor)
        sensor, i, temp = find_sensor(files[k], n)
...
for j, n in enumerate(sensor_number[c:d+1]):
    j = j + c
    cmap = plt.get_cmap(cmaps[0])(np.linspace(0.1, 0.9, N + 1))
    k = k + a
    for k, T in enumerate(Temp[a:b+1]):
        sensor, i, temp = find_sensor(files[k], n)
    ...
    #plt.colorbar(cm.ScalarMappable(norm=norm, cmap=cmaps[j]), label='Temperature (C)', ax=plt.gca())
#plt.legend()
plt.xlabel('Time (s)')
